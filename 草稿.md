...
- 上上次总结
  - 梳理上周构造的高质量数据集生成的管线，调试并解决了前面使用本地Qwen30B进行语料增强的一些prompt缺陷，目前完善了整个“合法路径42条→由路径反向构造种子模板问题168条→Qwen30B模板问题语义扩充564条→查询模板实体ID回填（LLM模糊替换增强+业务关联顺序抽取）→最终筛得560条高质量Question-Path样本”，即合格的生成从508条扩展到560条
  - 根据完整的生成数据管线和数据集调整现有RL训练框架的代码，然后试跑了一下调试好新的代码直接跑了之前RL的训练，发现前20000步RL确实都学不到奖励值，还是得SFT冷启动Roberta先
  - 整理并同步更新新的数据结构下所有训练架构的说明文档，沉淀数据生成和训练的总结经验
  
- 上次总结

  - 梳理当前基于项目助理Agent的查询路由RL训练现状，然后跟王工确认后续的训练计划。确认围绕先调优SFT的冷启动训练，然后调整数据集，再进行PPO强化训练。在AI小组例会上也确认了需要明确基线与训练的任务目标，后续也将利用当前构造好的数据集分别在baseline（没有训练的项目助理Agent）、SFT、RL上分别跑出得分结果，并进行对比分析
  - 下午基于构造好的数据集调试了SFT训练框架的代码，这块解决了一些bug，基本SFT训练是没问题了，后续就是调优训练效果，以及研究一下SFT冷启动训练与RL训练的衔接和数据处理的知识
  
- 总结

  - 基本调试通了在已构建的新数据集上的SFT冷启动训练，目前比较惊喜的是，在当前高质量数据集上划分出来的测试集计算的路径匹配的准确率能够到达95%以上，猜测新数据集分布更合理、质量更高，所以直接监督微调训练效果非常好。之前采用RL直接去PPO训练很难稳定收敛，后续就可以直接使用冷启动模型重新实验强化路线，以提升泛化能力: 处理 SFT 未见过的问题表述、在线学习: 基于真实 MQL 执行结果反馈优化
  
  - 查了一下使用SFT冷启动之后再RL训练需要新的数据集，所以这里重新基于上周构建的数据生成管线来构造更新的数据，不过需要先对42条合法路径进行种子模板问题的构建，这里初步也构造出了一版针对RL阶段后训练的全新RL数据集
...
- [x] 或者直接上真实场景，使用AI表格构造路径查询的数据进行初步的RL训练实验（换为真实数据、和Roberta基础模型）业务场景：Router智能决策层
  ...
  - [ ] 并行内容有，构建**业务RL训练（Text-to-MQL策略）**的数据、基模、训练框架
    ...
    - [x] 重构一下项目文件路径
    - [ ] 奖励机制——少用正奖励，多用负奖励（尝试只有全对才有大奖，其他错误都是负奖），因为**正向奖励可能正在扼杀模型的探索潜力。**
    - [ ] 基模更换（bert、T5、Qwen）——难点在于SFT要作为前提
    - [ ] 还得使用GRPO进行RL训练实验
    - [x] 了解SFT与RL训练数据是否需要划分的问题
      - [ ] 新生成的数据集（测试集）需要在baseline、SFT、RL上分别跑出得分结果——下周二汇报展示
      - [ ] 目前已经实验了SFT在RL新数据上路径匹配ACC在98%，下面要看一下baseline逆向的路径匹配ACC是多少？（对比-阶段性可用结果、汇报）
    - [ ] 了解关于pass@k与pass@1是否有训练上的差异
    - [ ] 了解新的三头actor与gym单头有什么区别和影响
      - [ ] 了解为什么当前初步的三头RL训练在500步之后奖励就崩了（刚好样本集也是500多）


- [ ] 或者还是并行，模拟实验继续探索其他RL算法（反正框架也搭好了，只需要微调部分板块即可）
  可快速产出可视化结果<br>- 算法对比有图有故事（上级关心换算法能否解决当前汇报过的问题-如reward hacking和不收敛）<br>- 操作熟练度提升
  **- 周二汇报要讲成一个“有头有尾的”故事（任务是什么、具体要干什么、结果怎么样、后续思考）**



![image-20251202084137193](https://raw.githubusercontent.com/Leon-Algo/leon_obsidian/main/picgo/image-20251202084137193.png)



![image-20251202164830877](https://raw.githubusercontent.com/Leon-Algo/leon_obsidian/main/picgo/image-20251202164830877.png)

![image-20251203163143723](https://raw.githubusercontent.com/Leon-Algo/leon_obsidian/main/picgo/image-20251203163143723.png)



![image-20251203172219489](https://raw.githubusercontent.com/Leon-Algo/leon_obsidian/main/picgo/image-20251203172219489.png)

![image-20251203172403176](https://raw.githubusercontent.com/Leon-Algo/leon_obsidian/main/picgo/image-20251203172403176.png)
